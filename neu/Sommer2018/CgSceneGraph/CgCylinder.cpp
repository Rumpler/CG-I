...
void CgCylinder::makeCylinder(int amountOfSegments, double height, double radius)
{
    ...
    for(int i = 0; i < amountOfSegments; i++){
        ...
        m_vertices.push_back(CgU::rotatePointYAxis(currentAngle, m_vertices.at(iBottomEdge)));
        m_vertices.push_back(CgU::rotatePointYAxis(currentAngle, m_vertices.at(iTopEdge)));
        ...
        if(i == 0){                 //FIRST LOOP
            ...
            //right
            fpr = CgU::calcFocusPointTriangle(m_vertices.at(iNextBot), m_vertices.at(iLastBot), m_vertices.at(iLastTop));
            nr  = CgU::calcFaceNormal(m_vertices.at(iNextBot), m_vertices.at(iLastBot), m_vertices.at(iLastTop));
            nrP = fpr + nr * 0.1f;
            m_face_normals.push_back(nr);
            //left
            fpl  = CgU::calcFocusPointTriangle(m_vertices.at(iNextBot), m_vertices.at(iLastTop), m_vertices.at(iNextTop));
            nl   = CgU::calcFaceNormal(m_vertices.at(iNextBot), m_vertices.at(iLastTop), m_vertices.at(iNextTop));
            nlP  = fpl + nl * 0.1f;
            m_face_normals.push_back(nl);
            //bottom
            fpb  = CgU::calcFocusPointTriangle(m_vertices.at(iBottomCenter), m_vertices.at(iLastBot), m_vertices.at(iNextBot));
            nb   = CgU::calcFaceNormal(m_vertices.at(iBottomCenter), m_vertices.at(iLastBot), m_vertices.at(iNextBot));
            nbP  = fpb + nb * 0.1f;
            m_face_normals.push_back(nb);
            //top
            fpt  = CgU::calcFocusPointTriangle(m_vertices.at(iTopCenter), m_vertices.at(iNextTop), m_vertices.at(iLastTop));
            nt   = CgU::calcFaceNormal(m_vertices.at(iTopCenter), m_vertices.at(iNextTop), m_vertices.at(iLastTop));
            ntP  = fpt + nt * 0.1f;
            ...
            glm::vec3 focus =  CgU::rotatePointYAxis(currentAngle, fpr);
            normalP = CgU::rotatePointYAxis(currentAngle, fpr + nr);
            ...
            m_vertex_normals.push_back(glm::normalize(CgU::rotatePointYAxis(-currentAngle,vnbP) - m_vertices.at(2))); //first edge point normals
            m_vertex_normals.push_back(glm::normalize(CgU::rotatePointYAxis(-currentAngle,vntP) - m_vertices.at(3))); //first edge point normals
            ...
            //right
            focus =  CgU::rotatePointYAxis(currentAngle, fpr);
            normal = CgU::rotatePointYAxis(currentAngle, fpr + nr) - focus;
            normal = glm::normalize(normal);
            normalP = focus + normal * 0.1f;
            m_face_normals.push_back(normal);
            //left
            focus =  CgU::rotatePointYAxis(currentAngle, fpl);
            normal = CgU::rotatePointYAxis(currentAngle, fpl + nl) - focus;
            normal = glm::normalize(normal);
            normalP = focus + normal * 0.1f;
            m_face_normals.push_back(normal);
            //top
            focus =  CgU::rotatePointYAxis(currentAngle, fpt);
            normal = CgU::rotatePointYAxis(currentAngle, fpt + nt) - focus;
            normal = glm::normalize(normal);
            normalP = focus + normal * 0.1f;
            m_face_normals.push_back(normal);
            //bottom
            focus =  CgU::rotatePointYAxis(currentAngle, fpb);
            normal = CgU::rotatePointYAxis(currentAngle, fpb + nb) - focus;
            normal = glm::normalize(normal);
            normalP = focus + normal * 0.1f;
            m_face_normals.push_back(normal);
            ...
            //top
            normal = CgU::rotatePointYAxis(currentAngle, m_vertices.at(iTopEdge) + vnt) - CgU::rotatePointYAxis(currentAngle, m_vertices.at(iTopEdge));
            normal = glm::normalize(normal);
            normalP = CgU::rotatePointYAxis(currentAngle, vntP);
            m_vertex_normals.push_back(normal);
            //bottom
            normal = CgU::rotatePointYAxis(currentAngle, m_vertices.at(iBottomEdge) + vnb) - CgU::rotatePointYAxis(currentAngle, m_vertices.at(iBottomEdge));
            normal = glm::normalize(normal);
            normalP = CgU::rotatePointYAxis(currentAngle, vnbP);
            m_vertex_normals.push_back(normal);
        }
