...
void CgSceneControl::rotate(glm::vec3 axis, float angle)
{
    m_current_transformation = glm::rotate(m_current_transformation, (float)((2.0f * M_PI / 360.0f) * angle), axis);
}
...
//        std::cout << *ev <<std::endl;
        ...
        if(ev->key() == Cg::Key_X){
            rotate(glm::vec3(1.0f,0.0f,0.0f), 45.0f);
            m_renderer->redraw();
        }
        if(ev->key() == Cg::Key_Y){
            rotate(glm::vec3(0.0f,1.0f,0.0f), 45.0f);
            m_renderer->redraw();
        }
        if(ev->key() == Cg::Key_Z){
            rotate(glm::vec3(0.0f,0.0f,1.0f), 45.0f);
            m_renderer->redraw();
        }
        if(ev->key() == Cg::Key_R){
            reset();
